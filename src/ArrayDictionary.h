...
public:
    ...
    bool contain(KeyType const &key) override {
        ...
        for(int i = 0; i < capacity; i++)
        {
            ...
            // If there is no entry for current hash key, jump to end of loop,
            if(ptr == nullptr)
            {
                continue;
            }

            if(ptr->key == key) // Condition runs if specified key is found.
            {
                if(ptr->value)
                {
                    return true; // Key has a value, return true.
                }
                else
                {
        ...
        return false; // After searching through dict, and specified key not found, return false.
    }
